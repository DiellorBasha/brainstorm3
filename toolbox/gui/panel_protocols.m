...
function isCopied = CopyNode( bstNodes, isCut )
    ...
    if ismember(nodeType{1}, {'channel', 'anatomy', 'volatlas', 'volct', 'volpet', 'noisecov', 'ndatacov'}) && (length(bstNodes) > 1)
...
function destFile = PasteNode( targetNode )
    ...
    isAnatomy = ismember(firstSrcType, {'anatomy','volatlas','volct','volpet','cortex','scalp','innerskull','outerskull','fibers','fem','other'});
    ...
    for i = 1:length(srcNodes)
        ...
        if (isCut || ismember(srcType, {'channel', 'noisecov', 'ndatacov', 'anatomy', 'volatlas', 'volct', 'volpet'})) && (iSrcStudy == iTarget)
...
function destFile = CopyFile(iTarget, srcFile, srcType, iSrcStudy, sSubjectTargetRaw)
    ...
    isAnatomy = ismember(srcType, {'anatomy','volatlas','volct','volept','cortex','scalp','innerskull','outerskull','fibers','fem','other'});
    % Get source subject
    if ~isAnatomy
