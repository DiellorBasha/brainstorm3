function [sPet, vox2ras, tReorient] = in_pet_nii(PetFile, isReadMulti, isApply, isScale)
% IN_PET_NII: Reads a structural NIfTI/Analyze PET.
%
% USAGE:  [sPet, vox2ras, tReorient] = in_pet_nii(PetFile, isReadMulti=0, isApply=[ask], isScale=[]);
%
% INPUT: 
%    - PetFile     : name of file to open, WITH EXTENSION
...
% ===== READ FILE =====
[filepath, baseName, extension] = bst_fileparts(PetFile);
switch(lower(extension))               
    % NIFTI-1 (single file) : documentation at http://nifti.nimh.nih.gov/nifti-1/
    case '.nii'
        fid = nifti_open_hdr(PetFile);
        if fid==-1, disp(sprintf('in_pet_nii : Error opening header file')); return; end
        % Read file header
        hdr = nifti_read_hdr(fid, isReadMulti);
        if isempty(hdr), disp(sprintf('in_pet_nii : Error reading header file')); return; end
        ...
        fclose(fid);
        if isempty(data), disp('in_pet_nii : Error reading image file'); return; end
    ...
    case {'.hdr', '.img'}
        ...
        if ~file_exist(hdrFilename), disp(sprintf('in_pet_nii : Missing header file (%s)', hdrFilename)); return; end
        if ~file_exist(imgFilename), disp(sprintf('in_pet_nii : Missing image file (%s)', imgFilename)); return; end
        % Read header file
        ...
        if fid == -1, disp(sprintf('in_pet_nii : %s', message)); return; end
...
% Brainstorm PET structure
sPet = db_template('petmat');
sPet.Cube    = data;
...
if ~isempty(hdr.nifti) && ~isempty(hdr.nifti.vox2ras)
    [sPet, vox2ras, tReorient] = cs_nii2bst(sPet, hdr.nifti.vox2ras, isApply);
end
...
%  =================================================================================================
%% ===== OPEN HEADER =====
function [fid, byteOrder] = nifti_open_hdr(PetFile)
    % Open file for reading only (trying little endian byte order)
    [fid, message] = fopen(PetFile, 'r', 'ieee-le');
    if fid == -1, disp(sprintf('in_pet_nii : %s', message)); return; end
...
function hdr = nifti_read_hdr(fid, isReadMulti)
    ...
    if ~isempty(nifti)
        ...
        % mainly just copied CH's code from petio.c
        b = nifti.quatern_b;
        c = nifti.quatern_c;
        d = nifti.quatern_d;
        x = nifti.qoffset_x;
