...
switch lower(structureName)
    ...
    case 'manifoldmat'
        template = struct(...
              ...
              'Extrinsic',     [], ...
              'Gauge',         [], ...
              'DEC',           [], ...   % 1x2 per-hemi struct: discrete exterior calculus primitives of the manifold -- d0 [E x V], d1 [F x E] (exterior derivatives); h0 [V x V], h1 [E x E], h2 [F x F] (Hodge stars, *2=diag(1/area)); sharp [3F x E] (musical iso #, 1-form->tangent face vector). toolbox/differential composes them on the de Rham complex L0->d0->L1->d1->L2: grad=#d0; div=-*0^-1 d0' #' W_F (W_F from *2); curl v=-div(N x v) (surface Hodge rotation). The metric flat ⋆1^-1#'W_F is NOT stored (⋆1 cotan has negatives -> unstable). Also carries GlobalVertices/GlobalFaces like the other facet groups.
              'Provenance',    []);
    ...
    case 'scout'
        template = struct(...
            ...
            'Handles',     repmat(struct( ...
                ...
                'hPatch',      [], ...
                'hContour',    [], ...
                'hGeodesic',   []), 0));   % cached line() handle of the geodesic polyline overlay
%     case 'scoutmat'
%         template = struct(...
%             'Vertices',   [], ... % Index of vertices
%             'Seed',       [], ... % Initial vertex of the scout area
